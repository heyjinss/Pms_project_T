...
//1. 언어선택 시 변경 처리 
						...
						$("#selectLan").change(function(){
							if( $(this).val()!=""){
								$(location).attr("href","${path}/choiceLan2.do?lang="+$(this).val()+"&login2="+login_auth_no);
							}
						});
					});		
</script>
									...
									<div class="row no-gutters align-items-center">
										<div class="col mr-2">
											...
											<c:forEach var="procnt" items="${procnt}">
												<div class="h5 mb-0 font-weight-bold text-gray-800">${procnt.procnt}</div>
											</c:forEach>
										</div>
										<div class="col-auto">
									...
									<div class="row no-gutters align-items-center">
										<div class="col mr-2">
											<div
												...
												<spring:message code="task" />
												<spring:message code="status" />
											</div>
											<c:forEach var="taskcnt" items="${taskcnt}">
												<div class="h5 mb-0 font-weight-bold text-gray-800">${taskcnt.taskcnt}</div>
											</c:forEach>
										</div>
										<div class="col-auto">
									...
									<div class="row no-gutters align-items-center">
										<div class="col mr-2">
											<div
												...
												<spring:message code="risk" />
												<spring:message code="status" />
											</div>
											<div class="row no-gutters align-items-center">
												<div>
													<c:forEach var="riskcnt" items="${riskcnt}">
														<div class="h5 mb-0 mr-3 font-weight-bold text-gray-800">${riskcnt.riskcnt}</div>
													</c:forEach>
												</div>
												<br>
									...
									<div class="row no-gutters align-items-center">
										<div class="col mr-2">
											<div
												...
												<spring:message code="looseends" />
												<spring:message code="project" />

											</div>
											<c:forEach var="dproject" items="${dproject}">
												<div class="h5 mb-0 font-weight-bold text-gray-800">${dproject.dproject}</div>
											</c:forEach>
										</div>
										<div class="col-auto">
					...
					<div class="row">
							...
							<div class="card shadow mb-4">
								...
								<div class="card-body">
									<c:forEach var="projectst" items="${projectst}">
										<h4 class="small font-weight-bold">${projectst.pro_name}
											<span class="float-right">${projectst.task2}</span>
										</h4>
										<div class="progress mb-4">
											<div class="progress-bar bg-danger" role="progressbar"
												style="width:${projectst.task2}%"
												aria-valuenow="${projectst.task2}" aria-valuemin="0"
												aria-valuemax="${projectst.tot}"></div>
										</div>
									</c:forEach>
								</div>
							</div>
						...
						<div class="row">
								...
								<div class="card shadow mb-6">
									...
									<div class="card-body">
										...
										<table class="table-bordered dataTable">
											<tr>
												<th><spring:message code="project" />
													<spring:message code="number" /></th>
												<th><spring:message code="project" />
											...
											<c:forEach var="pro" items="${projectlist}">
												<tr>
													<td>${pro.pro_no}</td>
													<td>${pro.pro_name}</td>
													<td><fmt:formatDate value="${pro.pro_enddte}" /></td>
													<td>지연</td>
												</tr>
											</c:forEach>
										</table>
									...
									<div class="card-body">
										<table class="table-bordered dataTable" width='100%'>
											...
											<c:forEach var="risktab" items="${risktab}">
												<tr>
													<td>${risktab.risk_name}</td>
													<td>${risktab.risk_no}</td>
													<td><fmt:formatDate value="${risktab.risk_regdte}" /></td>
													<td>${risktab.risk_state}</td>
												</tr>
											</c:forEach>
										</table>
									</div>
							...
							<div class="col-lg-4">
								...
								<input type="hidden" value="${graph_complete.graph_complete}"
									id="graph_complete"> <input type="hidden"
									value="${graph_delay.graph_delay}" id="graph_delay"> <input
									type="hidden" value="${graph_graphgoing.graph_graphgoing}"
									id="graph_graphgoing"> <input type="hidden"
									value="${graph_ready.graph_ready}" id="graph_ready">
							</div>
						</div>
					</div>
...
// Pie Chart Example
var fixing = document.getElementById("fixing");  //조치중 
var opening = document.getElementById("opening"); // 오픈 
var holding = document.getElementById("holding"); //홀드 
var complete = document.getElementById("complete"); //완료 
