...
//1. 프로젝트명 선택 시 테스크명 ajax로 호출 (task selectbox에 할당)
...
//2. 태스크이름 선택 시 ajax로 멤버번호 할당 (프로젝트별로 생성되는 mem_no가 다르므로)
...
//5. 파일처리 custom-file-input custom-file-label
		$(".custom-file-input").on("change",function(){
			$(this).next(".custom-file-label").text($(this).val());
		});
		
//5. selectbox로 선택한 값 옮기기
//5-1. selectbox - 프로젝트 
	...
	<div id="wrapper">
	  ...
	  <!-- DataTales Example -->
		...
		<!-- 정보 입력 영역 -->
									...
									<div class="col-sm-5">
										<input name="risk_name" value="${detail.risk_name}"
											class="form-control" />
									</div>
									...
									<div class="col-sm-5">
										<select
											class="custom-select custom-select-sm form-control form-control-sm  risk_kind">
											<option>유형</option>
											<option value="기술">기술</option>
											<option value="일정지연">일정지연</option>
										</select> <input type="hidden" value="" name="risk_kind" />
									</div>
								</div>
								<div class="form-group row">
									<label class="col-sm-1 col-form-label text-center">프로젝트</label>
									<div class="col-sm-5">
										<select name="projectname2" id="pro_no3"
											class="custom-select custom-select-sm form-control form-control-sm prono11">
											<option>프로젝트</option>
											<c:forEach var="projectlist" items="${projectlist}">
												<option value="${projectlist.pro_no}">${projectlist.pro_name}</option>
											</c:forEach>
										</select> <input type="hidden" value="" name="pro_no" />
									</div>
									<label class="col-sm-1 col-form-label text-center">작업명</label>
									<div class="col-sm-5">
										<select id="tasklist"
											class="custom-select custom-select-sm form-control form-control-sm tasklist">
										</select> <input type="hidden" name="task_no" class="form-control"
											value="" />
									</div>
								</div>
								<div class="form-group row">
									...
									<div class="col-sm-5" id="memname">
										<input name="mem_no" id="mem_no" class="form-control" value=""
											readonly />
									</div>
								...
								<div class="form-group row">
									...
									<div class="col-sm-5">
										<input class="form-control" placeholder="작성자입력하세요"
											value="${login.emp_korname}" readonly />
									</div>
								</div>
								<div class="form-group row">
									...
									<div class="col-sm-5">
										<input type="file" name="report" class="custom-file-input"
											placeholder="첨부파일을선택하세요" id="file01"> <input
											class="form-control" id="fname" value="${detail.report}" />
									</div>
								...
								<div class="form-group row">
									...
									<div class="col-sm-12">
										<textarea name="risk_content" rows="6" class="form-control"
